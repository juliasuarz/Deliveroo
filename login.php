...
<?php
    ...
    if (isset($_SESSION['incorrecto']) && $_SESSION['incorrecto']) {
        ...
        ?>
        <script>
        Swal.fire("Error", "La contraseña o usuario son incorrectos.", "error");
        </script>
        <?php
    }
    if (isset($_SESSION['vacio']) && $_SESSION['vacio']) {
        unset($_SESSION['vacio']); // Limpiar el indicador de la sesión

        // Mostrar SweetAlert
        ?>
        <script>
        Swal.fire("Error", "El campo mail es incorrecto.", "error");
        </script>
        <?php
    }
    if (isset($_SESSION['vacio2']) && $_SESSION['vacio2']) {
        unset($_SESSION['vacio2']); // Limpiar el indicador de la sesión

        ?>
        <script>
        Swal.fire("Error", "El campo contraseña esta vacio.", "error");
        </script>
        <?php
    }
    ...
    <div class="form-container">
        <div>
            <form action="validacion_login.php" method="post">
                ...
                <p style="text-align: center; width: 100%;">No tienes una cuenta? Inicia sesion <a href=" ./inc/singup.php" >aqui</a></p>
            </form>
        </div>
    </div>
</div>
